...
def build_lead_record(contact_payload, calculator_payload, entry_source=None, utm_payload=None, lead_type="construction"):
    ...
    # и формула) без изменений. Ниже добавлены только новые
    # ДОПОЛНИТЕЛЬНЫЕ поля структуры лида (Этап 5, п.10/14) — режим
    # использования и источник параметра (выбран клиентом или подобран
    # автоматически). Они не влияют на расчёт (calculate_price_estimate
    # уже отработал выше на исходном calculator_payload) и не читаются
    # ни Google Sheets (services/google_sheets.py берёт только заранее
    # известные ключи), ни Telegram (services/telegram.py — аналогично).
    ...
    return {
        **common_fields,
        ...
        # --- Новые поля (Этап 5) — только для внутренней структуры
        # лида, не участвуют в расчёте, не отправляются в Sheets/Telegram.
